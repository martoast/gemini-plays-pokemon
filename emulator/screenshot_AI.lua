...
function receiveControlInput()
    if statusSocket then
        ...
        if input then
            ...
            debugBuffer:print("Received raw input: " .. tostring(input) .. "\n")
            debugBuffer:print("Parsed keyIndex: " .. tostring(keyIndex) .. "\n")
            ...
            if keyIndex and keyIndex >= 0 and keyIndex <= 9 then
                ...
                debugBuffer:print("Adding key: " .. keyIndex .. " (" .. keyNames[keyIndex + 1] .. ")\n")
...
function socketReceived()
    while true do
        ...
        if p then
            ...
            local keyIndex = tonumber(p)
            if keyIndex and keyIndex >= 0 and keyIndex <= 9 then
                local keyNames = { "A", "B", "SELECT", "START", "RIGHT", "LEFT", "UP", "DOWN", "R", "L" }
                debugBuffer:print("Directly pressing key: " .. keyNames[keyIndex + 1] .. "\n")
                emu:clearKeys(0x3FF)  -- Clear all keys
                emu:addKey(keyIndex)  -- Press the received key
            end
        else
